...
namespace timer {
    ...
    */    
    ...
    public:
        ...
        explicit Timer(time_entry<> tickMs, int wheelSize, int tickerNum = 1,
                       time_entry<> timeout = time_entry<>({0,20000}), int threadNum = 2);
        ~Timer();
        void advanceClock();
        int size();
        /**
        ...
        */
        ...
        void addByDelay(const time_entry<>& delayTime, const Func& func, Args&& ... args) //-> decltype(func(std::forward<Args>(args)...))
        {
...
//        void addByDelay(const time_entry<>& delayTime, const Func& func) //-> decltype(func(std::forward<Args>(args)...))
//        {
//            auto f = func;
//            TimerTask* timerTask = new TimerTask(f);
//            time_entry<> tmp_now;
...
//        }
        ...
        */
        template<typename Func, typename ... Args>
        void addByExpiration(const TimerExpiration& expiration, const Func& func, Args&& ... args) {
            auto f = std::bind(func, std::forward<Args>(args)...);
            TimerTask* timerTask = new TimerTask(f);
...
//        template<typename Func>
//        void addByExpiration(const TimerExpiration& expiration, const Func& func) {
//            auto f = func;
//            TimerTask* timerTask = new TimerTask(f);
//            time_entry<> _expiration = expiration.getTimeEntry();
//            add(timerTask, _expiration);
//        }
         ...
         * newTimer的函数说明
         *
         * @brief newTimer 用于创建一个static Timer实例，用std::once_flag和std::call_once确保单例模式的线程安全
         * @param tickMs 一个时间轮一个刻度的时间间隔，类型为time_entry<>，精确到微秒
         * @param wheelSize 分级时间轮每一层时间轮所含刻度数目，类型为int
         * @param tickerNum 内部时钟的数目，时钟每次走动都会调用delayqueue.poll()
         * @param timeout 定时器内部时钟Ticker的触发间隔，类型time_entry<>，默认是{0,200000}
         ...
         *
        */
        static Timer * newTimer(time_entry<> tickMs, int wheelSize, int tickerNum = 2,
                                time_entry<> timeout = time_entry<>({0,20000}), int threadNum = 4);
