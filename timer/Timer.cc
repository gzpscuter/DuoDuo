...
Timer * Timer::newTimer(time_entry<> tickMs, int wheelSize, int tickerNum,
                        time_entry<> timeout , int threadNum )
{
    static std::once_flag flag;
    std::call_once(flag, [&]
    {
        assert(defaultTimer == nullptr);
        defaultTimer = new Timer(tickMs, wheelSize, tickerNum, timeout, threadNum);
        defaultTimer->start();
    });
    return defaultTimer;
}
